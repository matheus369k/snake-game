...
export function runningCurrentDirectionKeyBoard<T>(
  props: RunningCurrentDirectionKeyBoard<T>,
) {
  const { controlsKeys, keyBoardName } = props

  for (const controlKeys of controlsKeys) {
    const isCurrentDirection = controlKeys[1].some(
      (key) => key === keyBoardName,
    )
